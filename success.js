...
async function verifyPayment() {
  ...
  try {
    ...
    message.textContent = order.customerEmail
      ? `Stripe confirmed the payment. The receipt was sent to ${order.customerEmail}.`
      : "Stripe confirmed the payment.";

    order.items.forEach(item => {
      const row = document.createElement("li");
      row.textContent = `${item.quantity || 1} x ${item.name}`;
      items.append(row);
    });
  } catch (error) {
